...
class Models(object):
    def __init__(self,
        ...
        self.res_model = torchvision.models.resnet152(
            pretrained=False)  # res model for modal feature [1* 1000]
        self.res_model.load_state_dict(torch.load(config.root_path + '/model/resnet152-b121ed2d.pth'))
...
#         # 加载 bert 模型， 如果支持cuda, 则将模型加载到cuda中
    ...
    def feature_engineer(self):
        ...
        train = train.dropna(subset=['labelIndex']).reset_index(drop=True)
        test = test.dropna(subset=['labelIndex']).reset_index(drop=True)
        
#         # 统计每个类别的个数，看分布是否均匀
#         label_dict = {}
#         for idx in train['labelIndex'].tolist():
...
#         print(train['res_embedding'].tolist()[0])
#         print(test['res_embedding'])
...
#                 test['bow']))
        ...
        #  生成训练，测试的数据
#         cols = [x for x in train.columns if str(x) not in ['labelIndex', 'title', 'desc', 'label', 'queryCut', 'queryCutRMStopWord', 'queryCutRMStopWords', 'text', 
#                                                            'w2v', 'w2v_label_mean', 'w2v_label_max', 'w2v_mean', 'w2v_max', 'w2v_win_2_mean', 'w2v_win_3_mean', 
#                                                            'w2v_win_4_mean', 'w2v_win_2_max', 'w2v_win_3_max', 'w2v_win_4_max', 'res_embedding']]

        cols = [x for x in train.columns if str(x) not in ['labelIndex']]
        
#         test_cols = [x for x in test.columns if str(x) not in ['labelIndex', 'title', 'desc', 'label', 'queryCut', 'queryCutRMStopWord', 'queryCutRMStopWords', 'text', 
#                                                            'w2v', 'w2v_label_mean', 'w2v_label_max', 'w2v_mean', 'w2v_max', 'w2v_win_2_mean', 'w2v_win_3_mean', 
#                                                            'w2v_win_4_mean', 'w2v_win_2_max', 'w2v_win_3_max', 'w2v_win_4_max']]
        
        X_train = train[cols]
        print("训练集特征维度： " + str(X_train.shape))
#         print(X_train.info())
        
        X_test = test[cols]
        print("测试集特征维度： " + str(X_test.shape))
#         print(X_test.info())
    ...
    def unbalance_helper(self,
        ...
        model_name = 'lgb_w2v_mean_basic_res_bert_5000'
        ...
        # 训练， 并输出模型的结果
#         print('训练集特征维度：' + str(self.X_train.shape))
#         print('训练集标签维度' + str(self.y_train.shape))
...
#                                         tokenizer=self.ml_data.em.ae.tokenizer)
    ...
    def model_select(self, features, labels, CV=5):
        ...
        std_acc = cv_df.groupby('model_name').accuracy.std()
        
        acc = pd.concat([mean_acc, std_acc], axis=1, ignore_index=True)
