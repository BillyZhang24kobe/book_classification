...
class MLData(object):
    ...
    def preprocessor(self):
        ...
        self.dev = pd.read_csv(config.root_path + '/data/dev.csv',
                               sep='\t').dropna()
        if self.debug_mode:
            self.train = self.train.sample(n=5000, random_state=50).reset_index(drop=True)
            self.dev = self.dev.sample(n=500, random_state=50).reset_index(drop=True)
        # 拼接数据
    ...
    def process_data(self, method='word2vec'):
        ...
        '''
#         logger.info('load data')
#         train = pd.read_csv(config.root_path + '/data/train_clean.csv', sep='\t')
#         dev = pd.read_csv(config.root_path + '/data/dev_clean.csv', sep='\t')
...
#             json.dump({k: v for k, v in zip(self.category_id_df['label'], self.category_id_df['category_id'])}, f)
    ...
    def get_feature(self, data, method='word2vec'):
        ...
        '''
        if method == 'tfidf':
            dt = [' '.join(query) for query in data["queryCutRMStopWord"]]
            return self.em.tfidf.transform(dt)
